...
--Sequence created for autogenerating IDs
...
CREATE SEQUENCE tech_id_seq OWNED BY Technician.id;
select setval('tech_id_seq',  (SELECT MAX(id) FROM Technician));

CREATE SEQUENCE pilot_id_seq OWNED BY Pilot.id;
select setval('pilot_id_seq',  (SELECT MAX(id) FROM Pilot));

CREATE SEQUENCE plane_id_seq OWNED BY Plane.id;
select setval('plane_id_seq',  (SELECT MAX(id) FROM Plane));

CREATE SEQUENCE fnum_seq OWNED BY Flight.fnum;
select setval('fnum_seq',  (SELECT MAX(fnum) FROM Flight));

CREATE SEQUENCE fiid_seq OWNED BY FlightInfo.fiid;
select setval('fiid_seq',  (SELECT MAX(fiid) FROM FlightInfo));

CREATE SEQUENCE res_seq OWNED BY Reservation.rnum;
select setval('res_seq',  (SELECT MAX(rnum) FROM Reservation));
